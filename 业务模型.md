...
机器（当前：带 Docker 的 Debian 13 LXC，经私网 SSH 可达），并按预付费的资金账户计费。它是多租户
...
1. **为未来更多算力提供方保留灵活性。** 因为调用方只接触逻辑层，MicroCloud 以后完全可以把一个「机型」
   背后换成别的东西——Proxmox 的 **VM**（而非 LXC）、**外部云**实例、或者更轻的、根本不依赖 PVE 的
   **纯 Docker** 主机——而调用方代码不需要任何改动。物理层只是抽象背后的实现细节。
...
- **模板（template）**——目录里的机器镜像（如 `debian13`，即我们的 Debian 13 + Docker + Claude Code
  镜像）。模板按 placement 上传（镜像被拷到该 placement 的存储上）；机器只能创建在其模板已存在的
  placement 上。
